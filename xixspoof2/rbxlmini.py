...
def rbxl_to_xml_minimal(data: bytes) -> str:
    ...
    for m in RE_URL.finditer(data): 
        ids.add(m.group(2).decode())
    for m in RE_WEB.finditer(data): 
    ...
    if not ids:
        raise RuntimeError("rbxlmini could not find animation IDs in RBXL. Save as .rbxlx and retry.")

    xml = ['<roblox version="4">']
    for aid in sorted(ids, key=lambda x: int(x)):
        xml.append(
            f'<Item class="Animation"><Properties><Content name="AnimationId">rbxassetid://{aid}</Content></Properties></Item>'
        )
    xml.append("</roblox>")
    return "\n".join(xml)
