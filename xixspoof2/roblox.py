...
def _make_session(cookie: str) -> requests.Session:
    ...
    # Be nice to the infra: retries for 429/5xx and transient network issues
    retry = Retry(
        total=5,
        connect=5,
        read=5,
        ...
        allowed_methods=False,           # retry on POST too
...
class RobloxAPI:
    ...
    def _refresh_xcsrf(self):
        # Standard pattern: hit logout to receive X-CSRF-TOKEN header
    ...
    def _post_with_xcsrf(self, url: str, **kwargs) -> requests.Response:
        # Try with current token; if 403 + new token, refresh and retry once.
    ...
    def up_primary(self, data: bytes, name: str, desc: str, group: int | None):
        ...
        files = {
            "fileContent": ("animation.rbxm", data, "application/octet-stream")
        }
        body = {
            "assetType": "Animation",
            "name": name,
        ...
        # IMPORTANT: 'request' must be JSON string field for this endpoint
    ...
    def upload(self, data: bytes, name: str, desc: str, group: int | None) -> int:
        # Try the modern endpoint, then legacy; both with X-CSRF + retries from adapter
        nid = self.up_primary(data, name, desc, group)
        if nid:
            return nid
        # Small pause before fallback to avoid hammering
        time.sleep(0.4)
